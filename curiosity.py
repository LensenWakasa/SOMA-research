...
class CuriosityEngine:
    ...
    def _build_signal(
        ...
        """
        Compute the gated curiosity score C(s_t).

        C(s_t) = H_epist^2 / H_total

        This form naturally gates curiosity:
        - High H_epist, low H_aleat → C near H_epist (full signal)
        - High H_aleat (noise) → C suppressed (don't chase noise)
        """
        # Use calibrated noise floor if available
        ...
        C = (H_epist ** 2) / (H_total + eps)

        # Normalise C to [0,1] using tanh squashing
        C_norm = math.tanh(C)
